 ...
 *
 * UI PASS: the header/cover/avatar/stat blocks were oversized and the avatar
 * overlapped the name + action buttons on smaller screens. Everything has
 * been scaled down and re-spaced so it fits cleanly with no overlap.
 * ---------------------------------------------------------------------------
 */
...
function InfoRow({ icon, label, T, color }) {
  ...
  return (
    <div className="flex items-center gap-2.5 text-[13px] py-1" style={{ color: color || T?.muted }}>
      <span className="flex-shrink-0 w-5 flex justify-center text-base">{icon}</span>
...
function StatBox({ value, label, T }) {
  return (
    <div className="text-center flex-1 py-2 group cursor-pointer transition-all hover:scale-105">
      <div className="text-lg font-bold tracking-tight" style={{ color: T?.text }}>
        {typeof value === "number" ? value.toLocaleString() : value}
      </div>
      <div className="text-[10px] font-semibold uppercase tracking-wider mt-0.5" style={{ color: T?.mutedMid }}>{label}</div>
    </div>
  );
}
...
function RankCard({ icon, value, label, T }) {
  ...
  return (
    <div
      className="flex-1 rounded-xl p-3 text-center transition-all duration-300 hover:shadow-lg hover:-translate-y-1 backdrop-blur-md"
      style={{ background: T?.inputBg, border: `1px solid ${T?.inputBorder}` }}
    >
      <div className="text-xl mb-1 drop-shadow-md">{icon}</div>
      <div className="text-xl font-black tracking-tighter" style={{ color: T?.text }}>#{value}</div>
      <div className="text-[10px] font-bold uppercase tracking-wider mt-1" style={{ color: T?.mutedMid }}>{label}</div>
    </div>
  );
}
...
function PostCard({ post, T, isOwner, authorUser, allPosts, setAllPosts, onDeletePost }) {
  ...
  return (
    <div
      className="rounded-2xl overflow-hidden mb-4 transition-all duration-300 hover:shadow-xl group"
      style={{ border: `1px solid ${T?.divider}`, background: T?.bg, boxShadow: "0 6px 20px rgba(0,0,0,0.04)" }}
    >
      <div className="flex items-center justify-between p-3 pb-1.5">
        <div className="flex items-center gap-2.5">
          <MiniAvatar name={authorName} avatarUrl={authorAvatarUrl} accentColor={accentColor} size={36} />
          <div>
            <div className="flex items-center gap-1.5">
              <span className="font-bold text-[14px]" style={{ color: T?.text }}>{authorName}</span>
              {authorVerified && (
                <div className="w-[14px] h-[14px] rounded-full bg-gradient-to-br from-cyan-400 to-purple-500 flex items-center justify-center text-white text-[8px] font-black shadow-sm">✓</div>
              )}
              {post.isLocal && (
            ...
            <div className="text-[11px] font-medium" style={{ color: T?.mutedMid }}>{post.time}</div>
          </div>
        </div>
      ...
      <p className="px-3 py-1.5 text-[14px] leading-relaxed whitespace-pre-wrap" style={{ color: T?.text }}>
        {post.content}
      </p>

      {post.type === "image" && post.imageUrl && (
        <div className="px-3 pb-2.5">
          <div className="rounded-xl overflow-hidden shadow-sm relative group-hover:shadow-md transition-shadow">
            <img src={post.imageUrl} alt="" className="w-full max-h-[260px] object-cover block transition-transform duration-500 hover:scale-105" />
          </div>
        </div>
      )}

      <div className="flex justify-between px-4 pb-2.5 text-[11px] font-medium" style={{ color: T?.mutedMid }}>
        <span className="flex items-center gap-1.5 bg-black/5 px-2 py-1 rounded-lg"><Eye size={13} /> {(post.views || 0).toLocaleString()} views</span>
      ...
      <div className="flex px-1.5 py-0.5">
        {[
          { icon: liked ? <Heart size={17} fill="#ec4899" color="#ec4899" /> : <Heart size={17} />, label: liked ? "Liked" : "Like", action: handleLike, activeColor: "#ec4899", active: liked },
          { icon: <MessageCircle size={17} color={showComments ? accentColor : undefined} />, label: "Comment", action: () => setShowComments(v => !v), activeColor: accentColor, active: showComments },
          { icon: saved ? <BookmarkCheck size={17} fill={accentColor} color={accentColor} /> : <Bookmark size={17} />, label: saved ? "Saved" : "Save", action: handleSave, activeColor: accentColor, active: saved },
          { icon: <Share2 size={17} />, label: "Share", action: () => {}, activeColor: accentColor, active: false },
        ].map((btn, i) => (
          <button
            key={i}
            onClick={btn.action}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl transition-all duration-200 hover:bg-black/5"
            style={{
      ...
      {showComments && (
        <div className="p-3 flex flex-col gap-3.5" style={{ borderTop: `1px solid ${T?.divider}` }}>
          {comments.length === 0 && <p className="text-[13px] text-center italic py-2" style={{ color: T?.mutedMid }}>No comments yet. Be the first!</p>}
          {comments.map((c) => (
            <div key={c.id} className="flex gap-3">
              <MiniAvatar name={c.name} accentColor={accentColor} size={30} />
              <div className="flex-1">
                <div className="rounded-2xl rounded-tl-none px-3.5 py-2 shadow-sm" style={{ background: T?.inputBg, border: `1px solid ${T?.inputBorder}` }}>
                  <div className="text-[12.5px] font-bold mb-0.5" style={{ color: T?.text }}>{c.name}</div>
                  <div className="text-[13.5px] leading-relaxed" style={{ color: T?.text }}>{c.text}</div>
                </div>
                <div className="flex gap-3 text-[11px] mt-1 ml-2 font-medium" style={{ color: T?.mutedMid }}>
                  <span>{c.time}</span>
                </div>
              </div>
            </div>
          ...
          <div className="flex gap-3 items-center mt-1">
            <MiniAvatar name="You" accentColor={accentColor} size={32} />
            <div className="flex-1 flex gap-2 items-center relative">
              <input
                value={commentText} onChange={(e) => setCommentText(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleComment()}
                placeholder="Write a comment..."
                className="flex-1 py-2 pl-4 pr-12 rounded-full text-[13.5px] shadow-sm transition-all focus:ring-2"
                style={{
              ...
              {commentText.trim() && (
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 font-bold text-[13.5px] px-3 py-1 rounded-full hover:bg-black/5 transition-colors"
...
function VideoTile({ post, T, isOwner, onDeletePost }) {
  return (
    ...
    >
      <img src={post.thumbnail} alt="" className="w-full h-[180px] object-cover block transition-transform duration-700 group-hover:scale-105" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-80 group-hover:opacity-100 transition-opacity" />

      <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1.5 border border-white/10">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3">
        <p className="m-0 text-[12px] font-medium text-white/90 leading-snug line-clamp-2 drop-shadow-md">{post.caption}</p>
      </div>
    </div>
  );
}
...
export default function Profile({
  ...
  return (
    <div className="min-h-full pb-16 font-sans antialiased" style={{ background: T?.bg, color: T?.text }}>
      {/* ══ STICKY HEADER ══ */}
      <div
        className="sticky top-0 px-4 py-2.5 flex items-center gap-3 z-20 backdrop-blur-xl bg-opacity-80 shadow-sm"
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <h2 className="m-0 text-[16px] font-extrabold truncate">{user.name}</h2>
          <p className="m-0 text-[11px] font-medium" style={{ color: T?.mutedMid }}>{allOwnPosts.length} posts</p>
        </div>
      </div>

      {/* ══ COVER + AVATAR ══ */}
      <div
        className="relative h-[150px] flex-shrink-0 w-full"
      ...
      >
        ...
        <div className="absolute -bottom-10 left-5">
          <div
            ...
            className="w-[84px] h-[84px] rounded-full overflow-hidden cursor-pointer flex items-center justify-center text-white text-2xl font-black transition-transform duration-300 hover:scale-105 z-10 relative"
            style={{
              border: `4px solid ${hasActiveStory ? accentColor : T?.bg}`,
              boxShadow: hasActiveStory ? `0 0 0 3px ${accentColor}55, 0 6px 22px rgba(0,0,0,0.3)` : "0 6px 22px rgba(0,0,0,0.3)",
          ...
          {user.isOnline && (
            <div
              className="absolute bottom-1 right-1 w-4 h-4 rounded-full bg-green-500 z-20 shadow-sm"
          ...
          {hasActiveStory && (
            <button
              onClick={onOpenStory}
              className="absolute -bottom-2 left-1/2 -translate-x-1/2 text-[10px] font-bold px-3 py-0.5 rounded-full whitespace-nowrap cursor-pointer z-20 transition-transform hover:scale-110 border-2"
              style={{
      ...
      <div className="px-5 pt-12 pb-5">
        <div className="flex justify-between items-start gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="m-0 text-[20px] font-black tracking-tight">{user.name}</h1>
              {user.isVerified && (
                <div className="w-[18px] h-[18px] rounded-full bg-gradient-to-br from-cyan-400 to-purple-500 flex items-center justify-center text-white text-[10px] font-black shadow-md flex-shrink-0">✓</div>
              )}
            </div>
            <p className="m-0 mt-0.5 text-[13px] font-medium" style={{ color: T?.mutedMid }}>{user.handle}</p>
          </div>
          ...
          <div className="flex gap-2 flex-shrink-0 mt-0.5">
            {isOwnProfile ? (
              <>
                <button
                  onClick={onEdit}
                  className="px-3.5 py-1.5 rounded-xl text-[13px] font-bold cursor-pointer transition-all hover:bg-black/5 hover:scale-105 active:scale-95 shadow-sm"
                ...
                {!user.isVerified && (
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl border-none text-white text-[13px] font-bold cursor-pointer flex items-center gap-1.5 transition-all hover:opacity-90 hover:shadow-lg hover:scale-105 active:scale-95 shadow-md"
              ...
              <>
                <button
                  ...
                  className="px-4 py-1.5 rounded-xl border-none text-white text-[13px] font-bold cursor-pointer transition-all hover:opacity-90 hover:shadow-lg hover:scale-105 active:scale-95 shadow-md"
                  style={{ background: accentColor }}
                >
                  💬 Message
                </button>
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl text-[13px] font-bold cursor-pointer flex items-center gap-1.5 transition-all hover:scale-105 active:scale-95 shadow-sm"
                  style={{
        ...
        {user.bio && <p className="mt-3 text-[14px] leading-relaxed font-medium" style={{ color: T?.text }}>{user.bio}</p>}

        <div className="flex mt-4 py-1.5 rounded-2xl shadow-sm" style={{ background: T?.inputBg, border: `1px solid ${T?.divider}` }}>
          <StatBox value={allOwnPosts.length} label="Posts" T={T} />
          <div className="w-[1px] my-3 opacity-50" style={{ background: T?.divider }} />
        ...
        <div className="flex flex-col gap-0.5 mt-4 px-1">
      ...
      <div className="flex sticky top-[54px] z-10 backdrop-blur-xl bg-opacity-90 shadow-sm" style={{ borderBottom: `1px solid ${T?.divider}`, background: `${T?.bg}ee` }}>
        {tabs.map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className="flex-1 py-3 bg-transparent border-none text-[13px] cursor-pointer transition-all duration-300 relative"
            style={{
      ...
      <div className="p-4 pt-5">
        {postsForTab.length === 0 ? (
          <div className="text-center py-14 flex flex-col items-center">
            <div className="w-16 h-16 rounded-full flex items-center justify-center text-3xl mb-3" style={{ background: T?.inputBg }}>
              {activeTab === "liked" ? "🤍" : activeTab === "saved" ? "🔖" : "📝"}
            </div>
            <p className="text-[14px] font-semibold" style={{ color: T?.mutedMid }}>
              No {activeTab} yet.
            </p>
          </div>
        ) : (
